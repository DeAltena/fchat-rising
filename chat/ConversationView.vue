    ...
    <div style="height:100%;display:flex;flex-direction:column;flex:1;margin:0 5px;position:relative" id="conversation">
        <div style="display:flex" v-if="isPrivate(conversation)" class="header">
            ...
            <div style="flex:1;position:relative;display:flex;flex-direction:column;user-select:text">
                ...
                <div style="overflow:auto;overflow-x:hidden;max-height:50px;user-select:text">
                    ...
                    <span v-show="conversation.character.statusText"> – <bbcode
                        :text="conversation.character.statusText"></bbcode></span>
                </div>
            </div>
        </div>
        <div v-else-if="isChannel(conversation)" class="header">
            <div style="display: flex; align-items: center;">
                <div style="flex: 1;">
                    <span v-show="conversation.channel.id.substr(0, 4) !== 'adh-'" class="fa fa-star"
                    ...
                    <a href="#" @click.prevent="descriptionExpanded = !descriptionExpanded" class="btn">
                        <span class="fa"
                              :class="{'fa-chevron-down': !descriptionExpanded, 'fa-chevron-up': descriptionExpanded}"></span>
                ...
                <div class="btn-toolbar">
                    <dropdown :keep-open="false" title="View"
                              :icon-class="{fas: true, 'fa-comments': conversation.mode === 'chat', 'fa-ad': conversation.mode === 'ads', 'fa-asterisk': conversation.mode === 'both'}"
                              wrap-class="btn-group views" link-class="btn btn-secondary dropdown-toggle"
                              v-show="(conversation.channel.mode == 'both')">
                        <button v-for="mode in modes" class="dropdown-item"
                                :class="{ selected: conversation.mode == mode }" type="button" @click="setMode(mode)">
                            {{ l('channel.mode.' + mode) }}
                        </button>
                    </dropdown>

                    <dropdown :keep-open="false" wrap-class="btn-group ads" link-style=""
                        ...
                        <button class="dropdown-item" type="button" @click="showAdSettings()">Edit Channel Ads...
                        </button>
                        <div class="dropdown-divider"></div>
                        <button class="dropdown-item" :class="{ selected: showNonMatchingAds }" type="button"
                                @click="toggleNonMatchingAds()">Show Incompatible Ads
                        </button>
        ...
        <div class="auto-ads" v-show="isAutopostingAds()">
            ...
            <a class="btn btn-sm btn-outline-primary renew-autoposts" @click="renewAutoPosting()"
               v-if="!adsRequireSetup">{{ l('admgr.renew') }}</a>
            ...
            <template v-for="message in messages">
                ...
                <span v-if="hasSFC(message) && message.sfc.action === 'report'" :key="'r' + message.id">
                    ...
                    <span v-show="!message.sfc.confirmed">
                        | <a href="#"
                             @click.prevent="message.sfc.action === 'report' && acceptReport(message.sfc)">{{ l('events.report.confirm') }}</a>
                    </span>
                </span>
            </template>
        </div>
        ...
        <bbcode-editor v-model="conversation.enteredText" @keydown="onKeyDown" :extras="extraButtons"
            ...
            <div class="bbcode-editor-controls">
                ...
                <ul class="nav nav-pills send-ads-switcher" v-if="isChannel(conversation)"
                    ...
                    <li class="nav-item"
                        v-show="((conversation.channel.mode === 'both') || (conversation.channel.mode === 'chat'))">
                        <a href="#"
                           :class="{active: !conversation.isSendingAds, disabled: (conversation.channel.mode != 'both') || (conversation.adManager.isActive())}"
        ...
        <manage-channel ref="manageDialog" v-if="isChannel(conversation)"
                        :channel="conversation.channel"></manage-channel>
        <ad-view ref="adViewer" v-if="isPrivate(conversation) && conversation.character"
